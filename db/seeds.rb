...
hillyer = User.create(password: pw, email: "herberhillyer@IndieExpo.com", bio: "Tinkerer. Mechanic. Inventor. Travel Journalist.", first_name: "Herbert", last_name: "Hillyer", image_url: "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501281512/sidney-perry-105613_edydkp.jpg")
hammond = User.create(password: pw, email: "johnhammond@IndieExpo.com", bio: "John Parker Hammond is a flamboyant venture capitalist from Scotland, building his fortune in Petticoat Lane, England. He also held parks in Kenya, Costa Rica, and other tropical locations.", first_name: "John",last_name: "Hammond", image_url: "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501455152/peasant-farmer-farmer-romania-botiza-47862_clhy9n.jpg")
...
terraforma = Campaign.create(title: "Terra Forma", goal: 3000000, creator_id: hammond.id, category_id: travel.id, tagline: 'Beautiful and exotic by design.', city: 'Isla Sorna', country: 'Costa Rica', duration: 52, overview: lorem, pitch: 'We are designing and building a tropical getaway island. Having scoured the world for the most-loved tourist locations, we will bring our favorite elements of each to our newly created island. Experience the Harbor of Rio de Janeiro, the Grand Canyon, Great Barrier Reef, Mount Everest, Paricutin, and Victoria Falls, without leaving the island.', body: lorem, image_urls: ["https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501456422/rio-de-janeiro-olympics-2016-niteroi-brazil-161212_o73gl6.jpg", "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501456424/horseshoe-bend-page-arizona-colorado-river-86703_cj5g9t.jpg", "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501456427/pexels-photo-518958_bt798e.jpg", "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501456428/kalle-k-242413_sp8ywz.jpg", "http://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501456430/victoria-falls-605911_taxeqq.jpg", "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501456432/the-crater-of-the-volcano-paricutin-2282490_gtt8jh.jpg"])
...
Contribution.create(user_id: guest.id, amount: 3575, campaign_id: terraforma.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: dogs.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: stressgames.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: studio.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: time.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: drones.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: levitating.id)
Contribution.create(user_id: hammond.id, amount: 113, campaign_id: bees.id)
...
Perk.create(campaign_id: terraforma.id, title: 'Souvenir Flip Flops', description: 'Prepare for our beautiful, untouched beaches', price: 4000, image_url: "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501457632/sandals-flip-flops-footwear-beach-40737_iakgdy.jpg")
Perk.create(campaign_id: terraforma.id, title: 'Pre-Openning Two Week Stay', description: 'Be the first to experience our carefully engineered paradise', price: 50000, image_url: "https://res.cloudinary.com/dy4gcvjff/image/upload/q_auto:eco/v1501457579/pexels-photo-248797_jnqzvp.jpg")
